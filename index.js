...
function processLastItem(stringList, callback) {
  const arrLen = stringList.length - 1;
  return callback(`${stringList[arrLen]}`);
}
...
function processSum(numberList, callback) {
  let totalSum = 0;
  const arrLen = numberList.length;
...
function getFullNames(runners) {
  ...
  runners.forEach(function (item) {
    lastNames.push(`${item.last_name}, ${item.first_name}`);
  });
  return lastNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnerInfo = [];
  runners.forEach(function (item) {
    if (item.shirt_size.toUpperCase() === tShirtSize.toUpperCase()) {
      runnerInfo.push(item);
    }
  });
  return runnerInfo;
}
...
function tallyUpDonations(runners) {
  const donationArr = [];
  runners.forEach(function (item) {
    donationArr.push(item.donation);
  });
  const sum = donationArr.reduce((accumulator, currentValue) => {
